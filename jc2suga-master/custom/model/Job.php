...
class Job
{
	...
	function addApply( $id )
	{
		...
		{
			$db  = GMList::getDB('job');
			$db->setData( $rec , 'apply_count' , $db->getData( $rec , 'apply_count' ) + 1 );
			self::setApplyPos( $db, $rec );
			$db->updateRecord( $rec );
	...
	function setApplyPos( $db, &$rec )
	{
		...
		$use_max = SystemUtil::convertBool($db->getData( $rec, 'use_max_apply' ));
		if($use_max)
		{
			if( $db->getData( $rec, 'apply_count' ) >= $db->getData( $rec, 'max_apply' ) )
			{ $apply_pos = true; }
		}

		$db->setData( $rec, 'apply_pos', $apply_pos );
	}
