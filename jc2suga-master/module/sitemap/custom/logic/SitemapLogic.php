	...
	function createXml( $file, $kind, $list )
	{
		...
		foreach( (array)$kind as $name=> $data ) {
			if( strlen($data['url']) > 0 )
			{
				...
				$file = "sitemap.xml";
				$file = $name.$file;
				fileexamin::createIndexXml($file);
				$buffer = '';
				...
				$buffer .= '  </url>'."\n";
				fileexamin::fileExamine(fileexamin::$path.$file,$buffer);
            }
		...
		$buffer = '';
		$buffer .= '</sitemapindex>'."\n";
		fileexamin::fileExamine(fileexamin::$path.self::$ixml,$buffer);
