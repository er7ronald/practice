...
class SitemapLogic
{
	...
	function create()
	{
        ...
        // $kind['interview_itype']	 = array( 'name' => 'インタビュー詳細',	'url' => 'index.php?app_controller=info&type=interview&id= ');
        // $kind['mid_itype'] = array( 'name' => '求人検索','url' => 'index.php?app_controller=info&type=mid&id= ');
        // $kind['fresh_itype']	= array( 'name' => '求人詳細','url' => 'index.php?app_controller=info&type=fresh&id=');
        // $kind['cUser_itype']	= array( 'name' => '企業詳細',	'url' => 'index.php?app_controller=info&type=cUser&id=');
	...
	function createXml( $file, $kind, $list )
	{
		...
		foreach( $kind as $name => $data ) {
			...
			{
                SystemUtil::fileExamine($file,$buffer);
				$buffer .= '  <url>'."\n";
...
//    {
// 	   $db =  GMList::getDB('iterview');
// 	   $table = cUserLogic::getTable( "interview",null,null,"nobody");
...
//    }
   ...
   function getUserList()
   {
	   ...
	   $table = cUserLogic::getTable( "cUser",null,null,"nobody");
	   $table = $db->sortTable( $table, 'regist', 'desc');
	   $table = $db->limitOffset( $table , 0 , 1000 );
