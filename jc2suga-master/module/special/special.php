<?php
	...
	class mod_special extends command_base
	{
		...
		function doAsyncUpdate( &$gm , $rec , $args ){
		...
		function drawSearchEmbed( &$gm , $rec , $args ){
		...
		function drawSearchEmbed4SP( &$gm , $rec , $args ){
			...
			if(!$_GET["add"] || $_GET["add"]==0){
				$this->addBuffer( $gm->getCCResult( $rec, '<div class="item_list search_item"><!--# async code embedSearch type='.$c_type.'&run=true&special='.$id.'&special_PAL[]=match+like&embedID=job_special&authority=(!--# get authority #--)&add=(!--# get add #--)&page_name=' . $pageName . ' #--><p data-async-loader>求人情報を読み込んでいます...</p></div>' ) );
			}else{
				$this->addBuffer('<div class="item_list search_item"></div>');
			}
		}
		 ...
		 *
		 */
		function drawLink4SP( &$gm, $rec, $args )
		{
